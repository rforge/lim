...
Read <-  function (file, verbose = FALSE, checkLinear = TRUE, remtabs = TRUE)
{
    if (is.null(file))
        return(NULL)
#-------------------------------------------------------------------------------
## internal function: find the start and end of a section - only 1st 4 char 
...
#-------------------------------------------------------------------------------
    findsection <- function(Names) {
        ...
        if (length(pos) > 1)
            stop("error: section is present more than once",
                Names)
        if (is.null(pos))
            return(NA)
...
#-------------------------------------------------------------------------------
## internal function: replace everything between pos[1] and pos[2]
#-------------------------------------------------------------------------------
        ...
        for (i in 1:length(Strings)) {
            ...
            if (i1 > 0 & i2 > 0) {
                ...
                Strings[i] <- gsub(pattern, replacement, Strings[i],
                  fixed = TRUE)
            } else if (i1>0 & i2 <0) {Strings[i] <- gsub(pos[1], replacement, Strings[i],
                  fixed = TRUE)
            } else if (i1<0 & i2 >0) Strings[i] <- gsub(pos[2], replacement, Strings[i],
                  fixed = TRUE)
...
#-------------------------------------------------------------------------------
## internal function: remove everything between pos[1] and pos[2]
#-------------------------------------------------------------------------------
    Remove <- function(Strings, pos, All = TRUE) Replace(Strings,
        "", pos, All = All)
#-------------------------------------------------------------------------------
## internal function: remove everything after first occurrence of pos
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
## internal function: split a sentence in a name and a value
#-------------------------------------------------------------------------------
    Splitvaluename <- function(Strings) {
        ...
        for (i in 1:nlen) {
            ...
            res <- rbind(res, data.frame(name = pp[[i]][1], val = vv,
                parnr = pa))
        }
        return(res)
    }
#-------------------------------------------------------------------------------
## internal functions: finding parameters, components, externals etc..
#-------------------------------------------------------------------------------
    ...
    Findflow <- function(string) {
        ...
        if (length(grep("FLOWTO(", ss, fixed = TRUE)) > 0) {
            ...
            if (any(!posreac[flownr])) stop(paste("Cannot expand", string,"some flows are bidirectional"))
            return(flownr)
        }
...
#-------------------------------------------------------------------------------
## internal function: splitting flows into components from and components to
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
## internal function: parsing items  (e.g. -1*A*rB)
#-------------------------------------------------------------------------------
                        ...
                        if (is.na(EE)) {
                          ...
                          if (is.na(RR)) {
                            ...
                            if (any(is.na(GG)) && !createcomp)
                             {stop(paste("in string", ss, "cannot find item",
                              SS))} else if (any(is.na(GG))) 
...
#-------------------------------------------------------------------------------
## internal function: splitting strings in left and right section
#-------------------------------------------------------------------------------
    Splitleftright <- function(string, del = "=", check = FALSE) {
        hasdel <- grep(del, string)
        if (check & length(hasdel) != length(string))
            stop(paste("cannot split in left and right: delimiter not found",
                del))
        leftright <- matrix(ncol = 2, nrow = length(string),
            data = string)
        for (i in hasdel) leftright[i, ] <- unlist(strsplit(string[i],
            del, fixed = TRUE))[1:2]
        colnames(leftright) <- c("left", "right")
        return(leftright)
    }
#-------------------------------------------------------------------------------
## internal function: merging strings on several lines
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
## internal function: if no values declared: creates an empty structure
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
## internal function: splitting an equation in a name and equation sector 
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
## internal function: splitting an equation in left and right section for = , >, and <  
#-------------------------------------------------------------------------------
    SplitEquation <- function(string) {
        ...
        type[equal] <- "="
        ii <- which (type=="FALSE")
        if (length(ii)>0) stop(paste("cannot proceed: following is not an equation: ",string[ii]))
        leftright <- cbind(leftright, type)
        colnames(leftright) <- c("left", "right", "type")
        return(leftright)
    }
#-------------------------------------------------------------------------------
## internal function: splitting inequalities containing [] in > and <  
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
## internal function: splitting reaction containing <-> into one -> and one <-
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
## internal function: parsing an equation in single items
#-------------------------------------------------------------------------------
    ParseEquation <- function(LR, pref = "EQ", names = paste(pref,
        1:length(LR), sep = "")) {
        parsed <- NULL
        for (i in 1:nrow(LR)) {
...
#-------------------------------------------------------------------------------
## internal function: parsing a line equation in single items
#-------------------------------------------------------------------------------
    ParseLine <- function(String, pref="var", names=paste(pref,
        1:length(String), sep = "")) {
        cost <- NULL
        for (i in 1:length(String)) {
            ...
            for (j in 1:length(ss)) cost <- rbind(cost, data.frame(name = names[i],
                nr = i, finditem(ss[j], ct = 1)))
        }
        return(cost)
    }
#-------------------------------------------------------------------------------
## internal function: checking variable declarations: should be a function of unknowns
#-------------------------------------------------------------------------------
...
# ---------------------------   Parameters    ----------------------------------
    ...
    if (length(paramsec) > 1) {
        parnames <- Splitleftright(Lines[paramsec[1]:paramsec[2]],
            "=")[, 1]
...
# ---------------------------   Components    ----------------------------------
    ...
    compsec <- findsection(c("STOC", "COMP", "DECI", "STAT",
        "UNKN"))
    if (length(compsec) > 1) {
        createcomp <- FALSE
        compnames <- Splitleftright(Lines[compsec[1]:compsec[2]],
            "=")[, 1]
...
# ---------------------------    Externals    ----------------------------------
    ...
    if (length(externsec) > 1) {
        externnames <- Splitleftright(Lines[externsec[1]:externsec[2]],
            "=")[, 1]
...
# ---------------------------    Variables    ----------------------------------
    ...
    if (length(varsec) > 1) {
        varnames <- Splitleftright(Lines[varsec[1]:varsec[2]],
            "=")[, 1]
...
# ---------------------------  Equalities (=)  ---------------------------------
    ...
    if (length(equisec) > 1) {
        ...
        else equis <- ParseEquation(SplitEquation(eqstrings),
            "eq")
    }
# --------------------------- Inequalities (>) ---------------------------------
    if (verbose)
...
Setup.liminput <- function (liminput, ...)
{
    ...
    if (Unknown == "F") {
        ...
        if (any(!liminput$posreac))ispos<-FALSE
    } else if (Unknown == "R") {
